# TODO

All project TODO items are tracked here. Please do not leave TODO comments in source files — add or update entries in this file instead.

- [x] Review cleaned HTML output (completed 2026-02-11).
- [ ] Audit accessibility of outputs.
- [ ] Add badge icons + compact view improvements.
- [ ] Investigate whethr we can inject advanced features in to our conversion process (such as adding a close-able toolbar at the top or bottom of the page).
- [ ] Invesitgate adding feature to process entire notebooks and output as heirachical folder outputs as ZIP files.
- [ ] Run browser validation / smoke tests.
- [ ] Re-design UI to look professional and accessable.

---

If you move a TODO from code into this file, consider adding a short code comment to indicate the task was centralized, e.g. `// todo: moved to TODO.md`.
