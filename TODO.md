...
- [x] [P2] Post-release: extract a shared Playwright/static-server helper for the repeated `createStaticServer()` pattern used across the browser smoke tests so server setup changes do not require editing dozens of files. (2026-05-26, added `Tests/playwright-server-helper.js`, migrated the repeated smoke/a11y/export harnesses, and validated the shared server lifecycle across the migrated suite.)
- [x] [P2] Post-release: consolidate fixture discovery on top of `Tests/fixtures.js` and remove ad hoc `fs.readdirSync('Tests')` scans from regression scripts so the suite has one canonical fixture policy. (2026-05-26, routed the remaining regression scripts through `discoverFixtureFiles()` / `resolveFixturePath()`, fixed quoted-printable OneNote marker detection in `Tests/fixtures.js`, and reran the migrated checks.)
- [x] [P2] Post-release: extract shared Node-test setup helpers for logging suppression, JSDOM/DOMParser bootstrapping, and common assertion utilities instead of repeating `setEnabled(false)` and local polyfills across many unit scripts. (2026-05-26, added `Tests/node-test-helper.js`, migrated the repeated Node setup/logging patterns across the unit and regression scripts, and centralized the shared HTML diff helpers used by the regression checks.)
- [x] [P2] Post-release: reduce test dependence on production globals such as `window.__getRuntime` and `window.__getWorkerManagerDiagnostics` by introducing explicit test harness helpers or dev-only adapters. (2026-05-27, added `Tests/playwright-runtime-harness.js`, migrated the diagnostics, worker-id, and markdown-export Playwright flows onto the harness, updated the inline export-format controls harness to use the documented dev hook surface, and verified that the only remaining legacy references under `Tests/` are compatibility fallbacks inside the shared harness.)
...
5. Finish reusable launch assets after the core `v0.2` stabilization work lands by completing this small-step checklist, in priority order:
