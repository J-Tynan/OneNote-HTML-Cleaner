# Test Tag List

29 March 2026 20:31

Below is a list of all the default tags in Microsoft OneNote:

- [ ] To Do (unchecked)

- [x] To Do (checked)

Important

Question

Remember for later

Definition

Highlight

Contact

Address

Phone number

Web site to visit

Idea

Password

Critical

Project A

Project B

Movie to see

Book to read

Remember for e-reader

Music to listen to

Source for article

Remember for blog

Discuss with <Person A> (unchecked)

Discuss with <Person A> (checked)

Discuss with <Person B> (unchecked)

Discuss with <Person B> (checked)

Discuss with manager (unchecked)

Discuss with manager (checked)

Send in email

Schedule meeting (unchecked)

Schedule meeting (checked)

Call back (unchecked)

Call back (checked)

To Do priority 1 (unchecked)

To Do priority 1 (checked)

Client request (unchecked)

Client request (checked)

Created with OneNote.